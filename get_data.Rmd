---
title: "Measuring Days to First Visit from Original Placement Date"
output: 
  html_document: 
    keep_md: yes
---
...
visit_reports_all <- tbl(conn, "VisitReports") %>%
  filter(isCurrentVersion == TRUE,
         ...
         !is.na(serviceReferralId),
         !is.na(date)) %>%
  select(date, visitLocationCounty, caseNumber, serviceReferralId) %>%
  as_data_frame()

first_visit_reports <- inner_join(service_referrals
           ,visit_reports
           ,by = c("caseNumber", "serviceReferralId" = "id")) %>%
